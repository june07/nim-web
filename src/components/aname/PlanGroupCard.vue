<template>
	<v-card :style="xs ? 'width: 90vw' : ''" class="d-flex flex-column h-100 mx-3" :class="{ ...selectedClass, 'mr-1': !xs }" :color="isSelected ? 'blue-darken-4' : undefined" @click="toggle" rounded="xl" :flat="!isSelected">
		...
		<v-card-title class="text-h5 font-weight-bold mt-16">
			{{ plan.name }}
		</v-card-title>
		...
		<v-card-text style="font-size: 0.65rem" class="ml-2 text-capitalize">
            ...
            <div class="text-caption font-weight-light font-italic mt-n6 mb-4 text-grey" :style="{ visibility: plan.suggestedPrice ? 'visible' : 'hidden' }">suggested price {{ Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(plan.suggestedPrice/100) }}</div>
			<v-sheet class="text-body-1 font-weight-light" height="100" color="transparent">
