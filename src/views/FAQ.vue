	...
	<v-container class="h-100 d-flex align-center justify-center flex-column" :class="smAndDown ? 'px-1' : ''" fluid :style="smAndDown ? 'margin-top: 50px' : 'margin-top: 200px'">
		...
		<v-responsive :width="smAndDown ? '100%' : '80%'" max-width="1200" class="mx-auto">
			<v-container class="mb-8" :class="smAndDown ? 'pa-0' : ''">
				<v-breadcrumbs :items="categoryBreadcrumbs" class="justify-center mb-6 pa-0" color="red-darken-4">
				...
				<v-row justify="center">
					<v-col>
						<v-expansion-panels v-model="store.faq.category" accordian inset density="compact">
							<v-expansion-panel v-for="(categoryFaqs, categoryName) in faqs" :key="categoryName" rounded="xl" bg-color="red-darken-4">
								<!-- category -->
									...
									<v-expansion-panels v-model="store.faq.question" accordian inset>
										<v-expansion-panel v-for="(faq, index) in categoryFaqs" :key="faq.id" rounded="xl">
											...
											<v-expansion-panel-text v-bind:class="{ mobileFont: smAndDown }">
												...
												<div class="faq-answer text-red-darken-4" v-html="faq.answer"></div>
											</v-expansion-panel-text>
										</v-expansion-panel>
									</v-expansion-panels>
								</v-expansion-panel-text>
				...
				<v-row>
					<v-col class="pt-1">
						<v-expansion-panels model-value="0" density="compact" readonly>
							<v-expansion-panel rounded="xl" bg-color="red-darken-4">
								<v-expansion-panel-title collapse-icon="contact_support" :class="smAndDown ? 'text-h4' : 'text-h2'">Ask</v-expansion-panel-title>
								<v-expansion-panel-text>
									<v-card rounded="xl">
										<v-card-title>Don't see your question answered here?</v-card-title>
										<v-card-subtitle>Check our help articles or use our contact page for even more assistance if needed.</v-card-subtitle>
										<v-card-text class="ml-8">
											<ul>
												<li><router-link to="/help">Help Page</router-link></li>
		...
		<v-container v-if="!smAndDown" fluid style="background-color: red-darken-4; min-height: 500px"></v-container>
		<v-spacer />
	</v-container>
</template>
<style>
